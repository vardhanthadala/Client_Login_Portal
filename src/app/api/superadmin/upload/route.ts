...
export async function POST(req: NextRequest) {
  try {
    ...
    // Use platform-level AWS credentials
    ...
    if (!accessKeyId || !secretAccessKey || !region || !bucketName) {
      return NextResponse.json({ error: "Super Admin storage is not configured." }, { status: 500 })
    }

    // Sanitize the region
    const cleanRegion = region.split(" ").pop()?.trim() || region

    // Initialize S3 client with Super Admin credentials
    ...
    const key = `superadmin-uploads/${token.id}/${Date.now()}-${filename}`
    ...
    return NextResponse.json({ 
      uploadUrl: signedUrl, 
      fileUrl: `https://${bucketName}.s3.${cleanRegion}.amazonaws.com/${key}` 
    })

  } catch (error) {
    console.error("Super Admin S3 presigned URL generation error:", error)
