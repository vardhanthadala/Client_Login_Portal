...
export default function ClientNotificationsTab() {
  const { notifications, markAsRead, markAllAsRead } = useClientNotifications()
            ...
            <AnimatePresence>
              {notifications.map((notif) => (
                <motion.div
                  ...
                  exit={{ opacity: 0, x: -20 }}
                  className={`p-6 flex gap-4 transition-colors relative ${notif.isRead ? 'bg-transparent' : 'bg-[#FAFAFA] dark:bg-[#161616]'}`}
                >
                  ...
                  {!notif.isRead && (
                    <div className="shrink-0 flex items-center justify-center pl-2">
                      <button 
                        ...
                        className="w-10 h-10 rounded-full flex items-center justify-center hover:bg-white dark:hover:bg-[#222] border border-transparent hover:border-[#E2E8F0] dark:hover:border-[#333] text-[#94A3B8] hover:text-[#10B981] transition-all shadow-sm group"
                        title="Mark as read"
                      >
